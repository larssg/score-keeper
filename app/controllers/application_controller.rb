class ApplicationController < ActionController::Base
  ...
  def domain_required
    ...
    redirect_to account_url(current_user.account.domain) and return false if logged_in? &&
      current_account.domain != account_subdomain

    # User not logged in and no domain exists with the current subdomain
    redirect_to public_root_url(host: account_domain) and return false if !logged_in? &&
  ...
  def current_game
    if @current_game.nil?
      ...
      @current_game ||=
        current_account.games.first(conditions: { id: session[:current_game_id] }) if session[:current_game_id]
