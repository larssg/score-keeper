...
class Match < ActiveRecord::Base
  ...
  def log
    ...
    account.logs.create(
      ...
      game: game,
      message:
        "%#{winner.memberships.collect(&:user_id).join('% and %')}% won #{winner.score} to #{loser.score} over %#{loser.memberships.collect(&:user_id).join('% and %')}%",
    ...
    if game.track_clean_sheets? && (teams.first.score == 0 || teams.last.score == 0)
      account.logs.create(
        ...
        game: game,
        message:
          "%#{winner.memberships.collect(&:user_id).join('% and %')}% kept a clean sheet against %#{loser.memberships.collect(&:user_id).join('% and %')}%",
        published_at: played_at
      )
    end
  end
