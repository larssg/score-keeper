...
BUCKET_NAME = ("%s-test" % AWS_ACCESS_KEY_ID).downcase
CALLING_FORMATS = [S3::CallingFormat::REGULAR, S3::CallingFormat::SUBDOMAIN]
...
class TC_AWSAuthConnectionTest < Test::Unit::TestCase
  ...
  def test_operations
    ...
    CALLING_FORMATS.each do |format|
      @conn.calling_format = format 
...
class TC_QueryStringAuthGeneratorTest < Test::Unit::TestCase
  ...
  def test_operations
    ...
    # subdomain: http://bucket.s3.amazonaws.com/key
    # testing pure vanity domains (http://<vanity domain>/key) is not covered here
    # but is possible with some additional setup (set the server in @generator to your vanity domain)
    CALLING_FORMATS.each do |format|
      # construct the server name based on the calling format
      case format 
      when S3::CallingFormat::SUBDOMAIN
        server = "#{BUCKET_NAME}.#{@generator.server}"
      when S3::CallingFormat::REGULAR
        server = @generator.server
      else
        throw ArgumentError.new("Invalid calling format")
      end

      @http = Net::HTTP.new(server, @generator.port)
      @generator.calling_format = format 
      ...
      # for listing all buckets, remove the subdomain
      @http = Net::HTTP.new(@generator.server, @generator.port)
