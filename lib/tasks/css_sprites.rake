...
class CssSprites
  ...
  def generate_css
    ...
    image_map.each do |image|
      rules <<
        ".sprite-#{image[:key]} { display: table-cell; display: inline-block; width: #{image[:dimensions][0]}px; height: #{image[:dimensions][1]}px; background: url('#{self.output}') -#{image[:offset][0]}px -#{image[:offset][1]}px no-repeat; }"
    end
    rules.join("\n")
  end
