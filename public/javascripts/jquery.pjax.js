...
jQuery.fn.pjax = function (container, options) {
  ...
  if (options) options.container = container;
  else options = $.isPlainObject(container) ? container : { container: container };
...
jQuery.pjax = function (options) {
  ...
  var defaults = {
    ...
    success: function (data) {
      ...
      if (!$.trim(data) || /<html/i.test(data)) return (window.location = options.url);
      ...
      if (!$.pjax.active) {
        $.pjax.active = true;
        window.history.replaceState({ pjax: true }, document.title, location.pathname);
      }
      ...
      if (options.data) state.url = options.url + "?" + $.param(options.data);
      ...
      if ((options.replace || options.push) && window._gaq) _gaq.push(["_trackPageview"]);
...
jQuery(window).bind("popstate", function (event) {
  ...
  if (jQuery.pjax == jQuery.noop) return;
